 ...
 if (!$('html').hasClass('is-builder')) {
  $(document).on('add.cards', function(event) {
   ...
   $(event.target).outerFind('[data-bg-video-slide]').each(function() {
    ...
    var $preview = $('<div class="err-background-video-preview"></div>').css({
     display: 'none',
    ...
    var $overlay = $(this).find('.err-overlay');
    ...
    if (playerName === 'youtube') {
     ...
     if (isDesktop && $.fn.YTPlayer && !$(this).find('.playerBox').length) {
      $('.container-slide', this)
       .before('<div class="err-background-video"></div>').prev()
       .YTPlayer({
        videoURL: videoId,
        containment: 'self',
        showControls: false,
      ...
      $(this).find('.image_wrapper .err-overlay').css('opacity', '0');
     }
    } else {
